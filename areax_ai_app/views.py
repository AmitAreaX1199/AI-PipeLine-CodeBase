...
from .agent_utils import initialize_gemini_model, initialize_agent, get_current_datetime_info, process_agent_request
from .agent_instructions import get_scheduling_agent_instructions, get_wellness_bot_instructions
import random
...
class Pw_BroadcastAgentAPI(APIView):
    def post(self, request, *args, **kwargs):
        ...
        from .agent_instructions import get_broadcast_agent_instructions
        instructions = get_broadcast_agent_instructions(favorite_topics)
        
        # Process the request using the utility function
        from .agent_utils import process_openai_request
        return process_openai_request(
            prompt=instructions,
            model_name="gpt-4",
            system_instruction="You are a highly skilled AI persona Agent Called PW_Broadcast Agent, reply based on user sentiment in complete sentences.",
            user_reference_number=request.data.get("user_reference_number", ""),
            user_email=request.data.get("user_email", "")
        )
...
class TogetherChatAPIView(APIView):
    ...
    def post(self, request):
        ...
        from .agent_instructions import get_plm_agent_instructions
        system_instruction = get_plm_agent_instructions()
        
        # Process the request using the utility function
        from .agent_utils import process_together_request
        return process_together_request(
            prompt=user_message,
...
class SchedulingAgentAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        from .agent_utils import get_current_datetime_info
        current_datetime, timezone = get_current_datetime_info()
        
        from .agent_instructions import get_scheduling_agent_instructions
        instructions = get_scheduling_agent_instructions(current_datetime, timezone)
        ...
        from .agent_utils import initialize_agent, process_agent_request
        agent = initialize_agent(instructions, tools=[calendar_tools], show_tool_calls=True)
        
        # Process the request and return the response
        return process_agent_request(
            message=message,
            agent=agent,
            log_model=SchedulingAssistantLog,
            user_reference_number=user_reference_number,
            user_email=user_email
        )
...
class WellnessBotAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        from .agent_instructions import get_wellness_bot_instructions
        instructions = get_wellness_bot_instructions()
        
        # Initialize the agent with instructions and no tools
        from .agent_utils import initialize_agent, process_agent_request
        agent = initialize_agent(instructions, tools=None, show_tool_calls=False)
        
        # Process the request and return the response
        return process_agent_request(
            message=message,
            agent=agent,
            log_model=WellnessBotLog,
            user_reference_number=user_reference_number,
            user_email=user_email
        )
